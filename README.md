# Instagram — 美麗イラスト

[← NOTEアーカイブへ戻る](https://github.com/mushoku-note-log/NOTE)

## アカウント情報（2026年8月23日時点）

- ハンドル：@50daiossan（表示名：美麗イラスト）
- URL：https://www.instagram.com/50daiossan/
- フォロワー：47、フォロー中：0
- 自己紹介：「AIと相談しながら、記事・画像・ラノベ・イラスト・動画・カスタムGPTを形にする。」（Patreonリンクをプロフィールに記載）

## 注記

ログインなしでは個別の投稿一覧・キャプションが取得できないため、プロフィール基本情報のみの記録です。

## 関連リンク（同一プロジェクトの他アカウント）

- [NOTE（本体アーカイブ・企画の全体像）](https://github.com/mushoku-note-log/NOTE)
- [公式サイト / AICoCreationLab](https://aico-creation-lab.musyokunoossan.chatgpt.site/)
- [Patreon / AI Co-Creation Lab | Japanese Digital Art](http://patreon.com/cw/AICoCreationLab)
- [Patreon](https://github.com/mushoku-note-log/SNS-Patreon)
- [Facebook](https://github.com/mushoku-note-log/SNS-Facebook)
- [Medium](https://github.com/mushoku-note-log/SNS-Medium)
- [X（旧Twitter）](https://github.com/mushoku-note-log/SNS-X)
- [ちちぷい（chichi-pui）](https://github.com/mushoku-note-log/SNS-chichipui)
- [Aipictors](https://github.com/mushoku-note-log/SNS-Aipictors)
- [Painter AI](https://github.com/mushoku-note-log/SNS-PainterAI)
- [SeaArt](https://github.com/mushoku-note-log/SNS-SeaArt)

## このアカウントについて

「無職のおっさん」note収益化実験プロジェクト（現アカウント表示名：無職のおっさんの黄昏AI創作）の一環として運用されているアカウントです。AIとの対話から、記事・画像・ラノベ・動画・漫画・カスタムGPTを制作しています。詳しい経緯は[本体アーカイブ](https://github.com/mushoku-note-log/NOTE)を参照してください。

---

## 著者

無職のおっさんの黄昏AI創作

## 著作権と利用範囲

本文の著作権は、著者に帰属します。本文の全部または一部を、著者の許可なく転載、再配布、公開、販売することはご遠慮ください。作品そのもの（イラスト・漫画・楽曲等）の著作権は各プラットフォームの投稿ページの表記に従います。
